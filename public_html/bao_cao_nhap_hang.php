...
$ngayfabc = date('Y-m-d');
	$bang_px = mysqli_query($conn,"SELECT px.ma_phieuxuat,px.ma_kh,px.ngay_xuat,px.ma_nv,kh.ten_kh,nv.ten_nv,sum(ctx1.sl*ctx1.don_gia) as tong_mh,px.phanloai,px.tien_km_don_hang,px.tien_ship_dh FROM phieu_xuat px 
	...
	if (isset($_POST['tu_ngay'])) {	
		...
		if ($tu_ngay != '' & $den_ngay !='' ) {
			$bang_px = mysqli_query($conn,"SELECT px.ma_phieuxuat,px.ma_kh,px.phanloai,px.tien_km_don_hang,px.tien_ship_dh,px.ngay_xuat,px.ma_nv,kh.ten_kh as ten_kh,nv.ten_nv,sum(ctx1.sl*ctx1.don_gia) as tong_mh FROM phieu_xuat px 
		join nhan_vien nv on px.ma_nv = nv.ma_nv
		join khach_hang kh on px.ma_kh = kh.ma_kh 
		join chitiet_px ctx1 on ctx1.ma_phieuxuat = px.ma_phieuxuat  
				Where px.ngay_xuat >= '$tu_ngay' and px.ngay_xuat <= '$den_ngay' and px.phanloai ='Phiếu nhập'
		...
		join khach_hang kh on px.ma_kh = kh.ma_kh 
		join chitiet_px ctx1 on ctx1.ma_phieuxuat = px.ma_phieuxuat  
				Where px.ngay_xuat >= '$tu_ngay' and px.phanloai ='Phiếu nhập'
				GROUP BY px.ma_phieuxuat order by px.ngay_xuat desc");
			foreach ($bang_px as $key => $px){
				$tongtienhang +=$px['tong_mh'];
			} 
			...
			<table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
				...
				<tbody>
					...
					<tr>
						...
						<td style="text-align: right;"><?php echo number_format($px['tien_ship_dh']) ?></td>
					...
					<!-- Modal Chi tiết xuất -->
						...
						<td style="text-align: center;">
							...
							<td style="text-align: center;">
							<a href="chon_phieu_xuat_in.php?ma_phieuxuat=<?php echo $px['ma_phieuxuat'] ?>" target="_blank" name="btnin"> <i class="fa fa-print fa-1x" aria-hidden="true"style="color: blue"></i></a>
						</td>
						<td style="text-align: center;">
