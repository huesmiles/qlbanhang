...
if (count($ma_hh)) {
	foreach ($ma_hh as $key => $value) {
		...
		if ($hd == 'them' && $hh) {
			...
			$_SESSION['xuathh'][$value]['anh_hh'] = $hh['anh_hh'];
			$_SESSION['xuathh'][$value]['ma_hh'] = $hh['ma_hh'];
			$_SESSION['xuathh'][$value]['ten_hh'] = $hh['ten_hh'];
			$_SESSION['xuathh'][$value]['sl'] = 1;			
			$_SESSION['xuathh'][$value]['don_gia'] = $hh['dongia_ban'];
		}
	}
}
if ($hd == 'xoa') {
	unset($_SESSION['xuathh'][$mahhxoa]);
}
if ($hd1 == 'add1') {
...
if ($hd2 == 'del1') {
	...
	// $_SESSION['xuathh'][$ma_hh2]['anh_hh'] = $hh2['anh_hh'];
	// 		$_SESSION['xuathh'][$ma_hh2]['ma_hh'] = $hh2['ma_hh'];
	// 		$_SESSION['xuathh'][$ma_hh2]['ten_hh'] = $hh2['ten_hh'];
			$_SESSION['xuathh'][$ma_hh2]['sl'] -= 1;
			// $_SESSION['xuathh'][$ma_hh2]['don_gia'] = $hh2['dongia_ban'];
}
if (isset($_POST['ma_hh_1'])) {
	...
	foreach ($ma_hh_1 as $key =>  $mh) {
		...
		if (isset($so_luong[$mh])) {
				$sl = $so_luong[$mh];
				$gia = $giahh[$mh];
				if (isset($_SESSION['xuathh'][$mh])) {
					$_SESSION['xuathh'][$mh]['sl'] = $sl;
					$_SESSION['xuathh'][$mh]['don_gia'] = $gia;
				}
			}
		}
	}
