<?php include 'dau-trang.php' ?>
		...
		<?php 
			if (isset($_GET['ma_kh'])) {
				$ma_kh = $_GET['ma_kh'];
				$dm = mysqli_query($conn,"SELECT ten_kh,mobile,dia_chi,email,facebook,zalo,sinhnhat,phanloai FROM khach_hang where ma_kh='$ma_kh'");
				...
				if (isset($_POST['ten_kh'])) {
					...
					if ($ten_kh!='') {
						$them_moi = mysqli_query($conn,"UPDATE khach_hang set ten_kh ='$ten_kh',mobile ='$mobile',dia_chi ='$dia_chi',email ='$email' where ma_kh='$ma_kh'");
						if ($them_moi) {								
  ...
  <div class="card-header bg-dark"  style="color: white">Sửa thông tin</div>
	  <div class="card-body">
		<form action="" method="POST">
			...
			<a href="khach_hang.php" class="btn btn-sm btn-outline-dark"  style="padding-left: 30px;padding-right: 30px; margin-top: 20px;">Không sửa</a>
		</form>
	</div>
</div>
</div>
