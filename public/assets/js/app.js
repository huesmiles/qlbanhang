...
app.controller('AppCctrl', function ($scope, $http) {
	$scope.get_khach_hang = function () {
		if ($scope.tenkh != '') {
			$http.get('http://localhost/qlbanhang/api/khach-hang.php?ten_kh=' + $scope.tenkh)
				.then(function (res) {
					$scope.khs = res.data;
				});
		}
	...
	$scope.get_ncc = function () {
		...
		if ($scope.tenncc != '') {
			$http.get('http://localhost/qlbanhang/api/nha-cc.php?ten_kh=' + $scope.tenncc)
	...
	$scope.get_hh = function () {
		...
		if ($scope.tenhh != '') {
			$http.get('http://localhost/qlbanhang/api/hang-hoa.php?ten_hh=' + $scope.tenhh)
				.then(function (reshh) {
