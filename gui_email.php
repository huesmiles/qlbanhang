...
{
	// echo "title: " . $title . ", content: " . $content . ", nTo: " . $nTo . ", mTo: " . $mTo . ", cc: " . $diachicc . "\n";
	try {
		//$mto: địa chỉ nhận mail; $nto: tên người nhận
		$nFrom = 'Phần mềm quản lí bán hàng';
		$mail             = new PHPMailer(true);
		$address 					= 'testmailer123s@gmail.com';
		$password 				= 'Testmailer123@';
		...
		echo "Got exception: {$e}, {$mail->ErrorInfo}";
		return false;
	}
}
...
<?php /*echo !extension_loaded('openssl') ? "> OpenSSL Not Available" : "> OpenSSL Available"; */ ?>
		...
		if (mysqli_num_rows($bangds_kh) > 0) {
			...
			foreach ($bangds_kh as $key => $kh) {
				if ($kh['email']) {
					...
					if (sendMail($mail_title, $kh_content, $kh['email'], $kh['email'])) {
						// sleep(3);
