		...
		<?php 
			if (isset($_GET['ma_phieuthu'])) {
				...
				if (isset($_POST['tien_thu'])) {
					...
					if ($tien_thu!='') {
						$them_moi = mysqli_query($conn,"UPDATE phieu_thu set tien_thu ='$tien_thu',noi_dung ='$noi_dung' where ma_phieuthu='$ma_phieuthu'");
						if ($them_moi) {								
...
<div class="card text-dark" style="max-width: 100%;">
  <div class="card-header bg-dark"  style="color: white">SỬA PHIẾU CHI</div>
  	<div class="card-body">
		...
		<div class="form-group">
			...
			<a href="phieu_chi.php" class="btn btn-sm btn-outline-dark">Không sửa</a>
		</form>
	</div>
</div>
</div>
